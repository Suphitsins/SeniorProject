...
def main(_argv):
    ...
    while True:
       ...
       """
        ...
        for track in tracks:
            boxs.append([track[0], track[1], track[2], track[3]])
            indexIDs.append(int(track[4]))
            memory[indexIDs[-1]] = boxs[-1]
        ...
        if FLAGS.count:
            if len(boxs) > 0:
                ...
                for bbox in boxs:
                    ...
                    text = "Shoes passed: {}".format(indexIDs[i])
                    cv2.putText(frame, f'Shoes passed: {indexIDs[i]}', (0 + 10, height- 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,0), 1, cv2.LINE_4)  
                    i += 1
                
                
            frame = Image.fromarray(frame)
            draw = ImageDraw.Draw(frame)
            draw.text((83,152), 'shoes %d' %(shoes), fill=(255, 255, 255),font=font)
            draw.text((1328,112), 'shoes %d' %(shoes2), fill=(255, 255, 255),font=font)
